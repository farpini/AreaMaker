...
namespace PolyComp
{
    ...
    public static class Poly2Clip
    {
        private static Clipper cp;

        public static List<PolygonMesh> Union (List<PolygonMesh> polygons)
        {
            if (polygons.Count == 0)
            {
            ...
            cp.StrictlySimple = true;

            // add the polygon paths
            for (int i = 0; i < polygons.Count; i++)
            {
                cp.AddPaths(polygons[i].GetBoundariesPoints(), PolyType.ptClip, true);
            }
            ...
            //DebugUtil.ClearLogConsole();
            //PrintSolution(solution);

            return GetPolygonsFromSolution(solution);
        }

        public static List<PolygonMesh> Union (List<PolygonMesh> polygon1, List<PolygonMesh> polygon2)
        {
            // create a new clipper
            cp = new Clipper();
            //cp.ReverseSolution = true;
            ...
            // add the polygon1 paths
            for (int i = 0; i < polygon1.Count; i++)
            {
                cp.AddPaths(polygon1[i].GetBoundariesPoints(), PolyType.ptClip, true);
            }

            // add the polygon2 paths
            for (int i = 0; i < polygon2.Count; i++)
            {
                cp.AddPaths(polygon2[i].GetBoundariesPoints(), PolyType.ptClip, true);
            }
        ...
        public static List<PolygonMesh> Difference (List<PolygonMesh> clipPolygons, List<PolygonMesh> subjectPolygons)
        {
            // create a new clipper
            cp = new Clipper();
            //cp.ReverseSolution = true;
            cp.StrictlySimple = true;
            //cp.PreserveCollinear = true;

            // add the clip polygon paths
            for (int i = 0; i < clipPolygons.Count; i++)
            {
                cp.AddPaths(clipPolygons[i].GetBoundariesPoints(), PolyType.ptClip, true);
            }

            // add the subject polygon paths
            for (int i = 0; i < subjectPolygons.Count; i++)
            {
                cp.AddPaths(subjectPolygons[i].GetBoundariesPoints(), PolyType.ptSubject, true);
            }
        ...
        public static List<PolygonMesh> Intersection (List<PolygonMesh> clipPolygons, List<PolygonMesh> subjectPolygons)
        {
            // create a new clipper
            cp = new Clipper();
            //cp.ReverseSolution = true;
        ...
        public static List<PolygonMesh> XOR (List<PolygonMesh> clipPolygons, List<PolygonMesh> subjectPolygons)
        {
            // create a new clipper
            cp = new Clipper();
            //cp.ReverseSolution = true;
        ...
        public static List<PolygonMesh> CleanPolygons (List<PolygonMesh> polys)
        {
            List<PolygonMesh> cleanedPolys = new List<PolygonMesh>();
            for (int i = 0; i < polys.Count; i++)
            {
                List<IntPoint> cleanedOutside = Clipper.CleanPolygon(polys[i].GetOutsidePoints());
                if (cleanedOutside.Count > 3)
                {
                    List<List<IntPoint>> cleanedHoles = new List<List<IntPoint>>();
                    for (int j = 0; j < polys[i].HolesCount; j++)
                    {
                        if (polys[i].Holes[j].GetPoints().Count > 3)
                        {
                            cleanedHoles.Add(Clipper.CleanPolygon(polys[i].Holes[j].GetPoints()));
                        }
                    }

                    PolygonMesh polyMesh = ScriptableObject.CreateInstance<PolygonMesh>();
                    polyMesh.CreateBoundaries(cleanedOutside, cleanedHoles);
                    cleanedPolys.Add(polyMesh);
                }
            }
        ...
        public static PolygonMesh CombineTouchingHoles(PolygonMesh poly)
        {
            List<List<IntPoint>> polyHoles = new List<List<IntPoint>>(poly.GetHolesPoints());
            List<List<IntPoint>> combinedHoles = new List<List<IntPoint>>();

            if (polyHoles.Count < 2)
            {
                combinedHoles = polyHoles;
            }
            else
            {
                while (polyHoles.Count > 0)
                {
                    for (int i = 1; i < polyHoles.Count; i++)
                    {
                        List<IntPoint> combinedHole = Clipper.CreateWeaklyPolyFromStrictlyPolys(polyHoles[0], polyHoles[i]);
                        if (combinedHole != null)
                        {
                            polyHoles.RemoveAt(i);
                            polyHoles.RemoveAt(0);
                            polyHoles.Insert(0, combinedHole);
                            i = 1;
                        }
                    }

                    combinedHoles.Add(polyHoles[0]);
                    polyHoles.RemoveAt(0);
                }
            }

            PolygonMesh polyMesh = ScriptableObject.CreateInstance<PolygonMesh>();
            polyMesh.CreateBoundaries(poly.GetOutsidePoints(), combinedHoles);
            return polyMesh;
        }

        public static bool IsPointInPolygon (Vector2 point, List<PolygonMesh> poly)
        {
            //int meshRendererMultiplier = Poly2Mesh.MeshRendererMultiplier;
            IntPoint intPoint = new IntPoint(point.x, point.y);

            // first check the overall outside boundary of polygon
            // since the function cannot check holes at the same time
            for (int i = 0; i < poly.Count; i++)
            {
                if (Clipper.PointInPolygon(intPoint, poly[i].Outside.GetPoints()) != 0)
                {
                    // now check if its inside holes
                    for (int j = 0; j < poly[i].HolesCount; j++)
                    {
                        if (Clipper.PointInPolygon(intPoint, poly[i].Holes[j].GetPoints()) != 0)
                        {
                            return false;
                        }
                    }
        ...
        public static bool IsPointInPolygon (Vector2Int point, List<PolygonMesh> poly)
        {
            //RectInt unitPointRect = MathAux.GetSquareFromCenterPoint(point, 1);
            //List<PolygonMesh> rectMesh = new List<PolygonMesh>() {
            //    PolygonComp.GenerateRectanglePolygon(unitPointRect, 1) };

            //List<PolygonMesh> result = Intersection(poly, rectMesh);
            //return (result.Count > 0);
            Debug.LogError("Not implemented yet.");
            return false;
        }

        public static bool IsPolygonIntersectPolygon (List<PolygonMesh> poly1, List<PolygonMesh> poly2)
        {
            return (Intersection(poly1, poly2).Count > 0);
        }

        public static bool IsPolygonInsidePolygon (List<PolygonMesh> poly1, List<PolygonMesh> poly2)
        {
            return (Difference(poly2, poly1).Count == 0);
        }
        
        /*
        private static void PrintSolution (PolyTree solution)
        {
            DebugUtil.ClearLogConsole();
            int count = 0;
            ...
            while (polyNode != null)
            {
                string node = "Node" + count + ": " + ((polyNode.IsHole) ? "Hole" : "NotHole") + "  childs: " + polyNode.ChildCount;
                node += "  orientation: " + GetOrientation(new List<IntPoint>(polyNode.Contour));
                Debug.Log(node);
        ...
        */
        ...
        private static List<PolygonMesh> GetPolygonsFromSolution (PolyTree solution)
        {
            ...
            while (polyNode != null)
            {
                ...
                {
                    Debug.LogError("class Poly2Clip GetPolygonsFromSolution : Some poly node is open.");
                }

                polyNode = polyNode.GetNext();
            }

            List<PolygonMesh> polys = new List<PolygonMesh>();

            // now gets the childs for each parent
            for (int i = 0; i < nonHoleNodes.Count; i++)
            {
                List<IntPoint> outBoundary = nonHoleNodes[i].Contour;
                List<List<IntPoint>> polyHoles = new List<List<IntPoint>>();

                List<PolyNode> childNodes = nonHoleNodes[i].Childs;
                for (int j = 0; j < childNodes.Count; j++)
                {
                    polyHoles.Add(childNodes[j].Contour);
                }

                PolygonMesh polyMesh = ScriptableObject.CreateInstance<PolygonMesh>();
                polyMesh.CreateBoundaries(outBoundary, polyHoles);
                polys.Add(polyMesh);
            }
        ...
        public static PathOrientation GetOrientation(List<IntPoint> path)
        {
            return Clipper.GetPathOrientation(path);
        }

        public static PathType GetPolygonType (List<IntPoint> path)
        {
            return Clipper.GetPathType(path);
        }
    }
}
...
namespace ClipperLib
{
  ...
  using cInt = Int32;
    ...
    public enum PathType {
        StrictlySimple, // do not contain touching vertices
        WeaklySimple  // contains touching vertices
    };
    ...
    public IntPoint(IntPoint pt)
    {
      this.X = pt.X; this.Y = pt.Y; this.Z = pt.Z;
        ...
        public IntPoint (Vector2Int pt)
        {
            this.X = pt.x; this.Y = pt.y;
        }

        public IntPoint (Vector3Int pt)
        {
            this.X = pt.x; this.Y = pt.y;
        }

        public IntPoint (Vector2 pt)
        {
#if use_int32
            this.X = (int)pt.x; this.Y = (int)pt.y;
#else
    this.X = (long)pt.x; this.Y = (long)pt.y;
#endif
        }

        public IntPoint (Vector3 pt)
        {
#if use_int32
            this.X = (int)pt.x; this.Y = (int)pt.y;
#else
    this.X = (long)pt.x; this.Y = (long)pt.y;
        ...
        public static IntPoint operator + (IntPoint a, cInt v)
        {
            return new IntPoint(a.X + v, a.Y + v);
        }
        ...
        public static explicit operator Vector2Int (IntPoint v)
        {
            throw new NotImplementedException();
        }

        public static Vector2Int ToVector2Int (IntPoint a)
        {
            return new Vector2Int((int)a.X, (int)a.Y);
        }

        public static Vector2 ToVector2 (IntPoint a)
        {
            return new Vector2(a.X, a.Y);
        }

        public static IntPoint ToPoint (Vector2Int a)
        {
            return new IntPoint(a.x, a.y);
        }

        public static IntPoint ToPoint (int2 a)
        {
            return new IntPoint(a.x, a.y);
        }

        public static IntPoint ToPoint (Vector2 a)
        {
            return new IntPoint((long)a.x, (long)a.y);
        }
    ...
    public struct IntRect
    {
        ...
        public static Rect ToRect (IntRect r)
        {
            return new Rect(new Vector2(r.left, r.top), new Vector2(r.right - r.left, r.bottom - r.top));
        }
    }
    ...
    public class Clipper : ClipperBase
    {
      ...
      //------------------------------------------------------------------------------
        ...
        //------------------------------------------------------------------------------
        public static PathOrientation GetPathOrientation (Path poly)
        {
            return (Orientation(poly)) ? PathOrientation.CounterClockwise : PathOrientation.Clockwise;
        }
        //------------------------------------------------------------------------------
        public static PathType GetPathType (Path poly)
        {
            List<IntPoint> verticesChecked = new List<IntPoint>();
            for (int i = 0; i < poly.Count; i++)
            {
                for (int j = 0; j < verticesChecked.Count; j++)
                {
                    if (verticesChecked[j].Equals(poly[i]))
                    {
                        return PathType.WeaklySimple;
                    }
                }

                verticesChecked.Add(poly[i]);
            }

            return PathType.StrictlySimple;
        }
        ...
        // this function do not check that poly1 and poly2 are strictly simple polygons
        public static Path CreateWeaklyPolyFromStrictlyPolys (Path poly1, Path poly2)
        {
            // check if paths are in the same orientation
            if (GetPathOrientation(poly1) != GetPathOrientation(poly2))
            {
                Debug.LogError("NOT IN THE SAME ORIENTATION");
                return null;
            }

            Vector2Int verticeIndexesTouched;

            // check if poly1 and poly2 have touching vertices
            if (!ArePathsVerticesTouch(poly1, poly2, out verticeIndexesTouched))
            {
                return null;
            }

            // creates the weakly poly path
            List<IntPoint> poly1Updated = new List<IntPoint>();
            poly1Updated.AddRange(poly1.GetRange(verticeIndexesTouched.x, poly1.Count - verticeIndexesTouched.x));
            if (verticeIndexesTouched.x > 0)
            {
                poly1Updated.AddRange(poly1.GetRange(0, verticeIndexesTouched.x));
            }

            List<IntPoint> poly2Updated = new List<IntPoint>();
            poly2Updated.AddRange(poly2.GetRange(verticeIndexesTouched.y, poly2.Count - verticeIndexesTouched.y));
            if (verticeIndexesTouched.y > 0)
            {
                poly2Updated.AddRange(poly2.GetRange(0, verticeIndexesTouched.y));
            }

            List<IntPoint> weaklyPolyOut = new List<IntPoint>(poly1Updated);
            weaklyPolyOut.AddRange(poly2Updated);

            return weaklyPolyOut;
        }

        public static bool ArePathsVerticesTouch (Path poly1, Path poly2, out Vector2Int firstTouchingPathsVerticesIndexesFound)
        {
            firstTouchingPathsVerticesIndexesFound = Vector2Int.zero;

            for (int i = 0; i < poly1.Count; i++)
            {
                IntPoint p1pt = poly1[i];
                for (int j = 0; j < poly2.Count; j++)
                {
                    if (p1pt.Equals(poly2[j]))
                    {
                        firstTouchingPathsVerticesIndexesFound = new Vector2Int(i, j);
                        return true;
                    }
                }
            }
